...
- **Pick up some of an item, not all of it**: a line asking for 2 cartons of milk can be ticked off a carton at a time. Every line for more than one unit carries a small **− 1/2 +** stepper on the shared shopping list: each press records what actually went into the basket, the row shows "1 of 2 cartons picked up", and the line only counts as bought once the whole amount is found. The tick box beside it is unchanged and still takes the whole line in one press, so nothing slows down for the items bought in one go. Lines measured in larger amounts step by a round tenth of the line (500 g moves 50 g at a time), so no line is ever more than about ten presses from finished. See [Picking Up Part of a Line](#picking-up-part-of-a-line).
...
- `PATCH /api/grocery/shared/:token/items/:itemId` — record how much of a line is in the basket. The body is either `{ "pickedUpQuantity": 1 }` for part of a line or `{ "isChecked": true }` for all of it; the amount is clamped to the line's own quantity and decides `isChecked`. No admin login, the share token is the credential. See [Picking Up Part of a Line](#picking-up-part-of-a-line).
...
## Picking Up Part of a Line

Shopping rarely matches the list exactly: the list says 2 cartons of milk and the shelf has one left. Before, the only answer was to tick the whole line off or leave all of it untouched, and neither says what is in the basket.

- Every `GroceryItem` carries a `pickedUpQuantity` alongside its `quantity`. `isChecked` is kept as well and means "all of it" — it is written from the amount rather than set on its own, so the two can never disagree.
- On the shared shopper link, every line for **more than one unit** shows a stepper: **−**, the amount so far as `1/2`, and **+**. A press records one step, the row explains itself in words ("1 of 2 cartons picked up"), and its tick box shows the same dash a browser draws for a half-ticked checkbox. A line for a single item keeps the plain tick box, since there is nothing to split.
- **The tick box still takes everything at once.** Ticking is simply the largest pick up there is — it writes the full amount — and unticking puts the whole line back. The stepper is for the times only part of it made it into the basket.
- **Step size follows the amount.** Lines up to 12 units step by one. Larger ones step by a round tenth of the line (500 g moves 50 g at a time, 20 pieces move 2 at a time) and the last press lands exactly on the full amount, so a line for 2.5 kg goes 1 kg, 2 kg, 2.5 kg. No line is more than about ten presses from finished, whatever it is measured in.
- **A half-finished line stays where it is.** Only lines that are fully picked up sink to the bottom of the list, because a line with one carton still to find is still shopping to do.
- The progress bar counts a half-picked line as half done — each line contributes its own fraction, so a line measured in grams cannot drown out one counted in cartons — and the header adds "· 2 partly picked up" beside the count of finished lines.
- On the plan's grocery page, both the merged shopping list and the detailed item list show what the shoppers have found so far as a "1 of 2 cartons picked up" badge. Amounts are recorded through the shared link, which is where the shopping happens, so this side of the app reports rather than offers its own stepper.
- **Editing an amount drags the picked up amount with it.** Cutting a line from 3 cartons to 1 after 2 were already found leaves it at 1 of 1 and finished, rather than claiming more was bought than was ever asked for; growing it again leaves what was found in place and reopens the line.
- Amounts are clamped server-side to the range the line allows and rounded, so a stray request cannot store a negative amount or more than the line asks for, and repeated steps on a fractional line cannot leave it a crumb short of done.
- Partial pick ups broadcast over the same realtime stream as everything else, so two people shopping from the same link see each other's cartons appear.
...
- **The token is looked up across the whole plan, not on a fixed day.** The grocery page requests the share link the moment it opens, so two requests for the same plan can both see "no token yet" and both try to create one; the second create used to fail the request with a unique-constraint error (`GroceryShareToken_planDayId_key`) instead of returning the token the first one had just written. The endpoint now catches that conflict and reads the winning token back. The lookup also searches every day of the plan rather than assuming the first one, so a plan extended backwards to an earlier start date reuses its existing token instead of minting a second one.
...
- A shopper can also tick off **part** of a line — one of the two cartons of milk — with the stepper beside the tick box. See [Picking Up Part of a Line](#picking-up-part-of-a-line).
- Token rotation is an explicit action via `POST /api/plans/:id/share-link/rotate`; rotating invalidates old links and limits continued access. Rotation replaces the plan's existing token row wherever it sits, rather than writing a fresh token onto the plan's first day — writing to a fixed day would leave a token stored on another day of the plan still valid, so the link being rotated away from would keep working.
